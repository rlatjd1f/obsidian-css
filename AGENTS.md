...
- Before adding a rule, check whether a LATER block already targets the same selector — in this file a later block commonly re-declares it with `!important` (the F1 theme is built as two stacked layers this way). A declaration that loses to a later `!important`, or a `body` variable that loses to a `.theme-*` class, is dead code that silently does nothing; both have already happened here. When a later layer owns a property, edit that layer instead of adding a competing rule.
